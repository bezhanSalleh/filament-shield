...
- [Usage \& Configuration](#usage--configuration)
  - [Permissions](#permissions)
    ...
    - [Customize permission label composition](#customize-permission-label-composition)
      - [Basic Usage](#basic-usage)
      - [Localized Labels](#localized-labels)
      - [Action-Specific Formatting](#action-specific-formatting)
      - [Entity Type-Based Formatting](#entity-type-based-formatting)
      - [Using Default Label as Base](#using-default-label-as-base)
  - [Policies](#policies)
...
### Customize permission label composition

Similar to permission keys, you can customize how permission labels are displayed throughout the Shield interface by providing your own callback to `buildPermissionLabelUsing` in your `AppServiceProvider`'s `boot()` method. This is particularly useful for internationalization, consistent branding, or business-specific terminology.

The callback receives the following parameters:

- `string $entity`: The fully qualified class name of the entity (Resource, Page, or Widget).
- `string $affix`: The permission affix (e.g., 'viewAny', 'create', 'update', 'delete').
- `string $subject`: The resolved subject name (e.g., 'User', 'Post', 'Dashboard').
- `string $defaultLabel`: The default label that would be generated (e.g., 'View User', 'Create Post').
...
FilamentShield::buildPermissionLabelUsing(function (string $entity, string $affix, string $subject, string $defaultLabel): string {
    return "Can {$affix} {$subject}";
});
```

This will change labels from "View User" to "Can view User", "Create Post" to "Can create Post", etc.

#### Localized Labels

For internationalization, you can use Laravel's localization features:

```php
FilamentShield::buildPermissionLabelUsing(function (string $entity, string $affix, string $subject, string $defaultLabel): string {
    return __('permissions.labels.format', [
        'action' => __("permissions.actions.{$affix}"),
        'subject' => $subject,
    ]);
});
```
...
You can provide different label formats for different actions:

```php
FilamentShield::buildPermissionLabelUsing(function (string $entity, string $affix, string $subject, string $defaultLabel): string {
    return match ($affix) {
        'viewAny' => "Browse {$subject} Records",
        'view' => "View Individual {$subject}",
        'create' => "Add New {$subject}",
        'update' => "Edit {$subject}",
        'delete' => "Remove {$subject}",
        'restore' => "Restore Deleted {$subject}",
        'forceDelete' => "Permanently Delete {$subject}",
        default => $defaultLabel,
    };
});
```

#### Entity Type-Based Formatting

You can customize labels based on the type of entity:

```php
FilamentShield::buildPermissionLabelUsing(function (string $entity, string $affix, string $subject, string $defaultLabel): string {
    $entityInstance = resolve($entity);

    if ($entityInstance instanceof \Filament\Resources\Resource) {
        return "Resource: {$affix} {$subject}";
    }

    if ($entityInstance instanceof \Filament\Pages\BasePage) {
        return "Page Access: {$subject}";
    }

    if ($entityInstance instanceof \Filament\Widgets\Widget) {
        return "Widget: View {$subject}";
    }
...
#### Using Default Label as Base

You can modify the default label while preserving the original logic:
...
The custom label builder affects labels throughout the Shield interface, including:

- Role management forms
- Permission selection interfaces
- Any place where permission labels are displayed
