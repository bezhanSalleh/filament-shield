...
## Permissions
You can customize how permission keys are generated to match your preferred naming conventions and organizational standards. Shield uses these settings from `filament-shield.php` **config** when creating permission names from your `{Resources|Page|Widgets}`.
...
* Now let's consider an example where we want to handle `Resource` entities that handles the same `Model` or `Models` with same name but with different namespace and directory structure. The Filament [Demo](https://github.com/filamentphp/demo) has two resources with the same name which handles two different models:
...
### Policy Enforcement
Laravel automatically resolves policies for models, but this is not always the case. For instance, if your models are not in the default `App\Models` namespace or nested or they are from third-party plugins, you may need to manually register the policies. You can do this in a service provider's `boot()` method: 
...
## Custom Permissions
Most of the time you will have some ad‑hoc permissions that don't fit into the `Resource`, `Page`, `Widget` categories or you might not want a policy method for it. You can define these under `custom_permissions` in the config:
```php
'custom_permissions' => [
...
The plugin exposes a couple of methods to handle its resource related customizations and overrides without publishing the resource. You can use the plugin as following:
