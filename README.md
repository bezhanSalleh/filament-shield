...
### Multi-panel permission isolation (optional)
If your app uses multiple panels that share the same auth guard, enable `permissions.panel_prefix` to avoid permission name collisions. When enabled, Shield prefixes every generated permission key (resources, pages, widgets, and custom permissions) with the current panel ID.
...
If your app uses multiple panels, you can enable `policies.panel_path` to store generated policies under a panel-specific subdirectory. The default panel continues to use the base path.
